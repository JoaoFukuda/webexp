...
namespace webexp
{
	namespace proxy
	{
		...
		void Socks4Proxy::handle_client(int sock) const {
			...
			std::cout << "\e[1;94m[INFO]\e[m Got request for"
				<< "\n\e[1;92m>\e[m Version: " << static_cast<int>(request.version)
				<< "\n\e[1;92m>\e[m Command: " << static_cast<int>(request.command)
				<< "\n\e[1;92m>\e[m Destination port: " << ntohs(request.dest_port)
				<< "\n\e[1;92m>\e[m Destination ip: " << inet_ntoa(addr) << std::endl;
			char buf[BUFSIZE];
			memset(buf, 0, BUFSIZE);
			recv(sock, buf, BUFSIZE, 0);
			std::cout << "\e[1;92m>\e[m ID: " << buf << std::endl;

			if (request.version != 4)
				throw("Wrong SOCKS proxy protocol version");
			...
			if (connect(server_sock, reinterpret_cast<sockaddr*>(&server_hint), sizeof(server_hint)) == -1) {
				std::cerr << "\e[1;91m[ERROR]\e[m Could not connect to server at " << inet_ntoa(addr) << ':' << ntohs(request.dest_port) << std::endl;
			...
			std::cerr << "\e[1;92m[SUCCESS]\e[m Connected to server at " << inet_ntoa(addr) << ':' << ntohs(request.dest_port) << std::endl;
		...
		void Socks4Proxy::handle_next_connection() const noexcept {
